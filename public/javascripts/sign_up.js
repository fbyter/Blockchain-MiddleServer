...
/* 绑定注册按钮 */
document.getElementById('input_ok').addEventListener('click', function () {
  ...
  ajaxObj.open('post', url+'/register');
  ...
  ajaxObj.onreadystatechange = function () {
    ...
    if (ajaxObj.readyState === 4 && ajaxObj.status === 200) {
      ...
      if(response.status === 'success') {
        alert(response.msg);
        window.location.href = url + `/signin?name=${args.name}&password=${args.password}`;
      } else {
        alert(response.msg)
      }
